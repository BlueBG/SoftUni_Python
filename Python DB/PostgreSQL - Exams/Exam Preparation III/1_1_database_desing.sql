...
CREATE TABLE  IF NOT EXISTS addresses (
    ...
    CONSTRAINT street_number_ck CHECK (street_number > 0),
    CONSTRAINT zip_code_ck CHECK (zip_code > 0)
);
...
CREATE TABLE  IF NOT EXISTS players_ranges (
    ...
    CONSTRAINT min_players_ck CHECK (min_players > 0),
    CONSTRAINT max_players_ck CHECK (max_players > 0)
);
...
CREATE TABLE IF NOT EXISTS board_games (
    ...
    CONSTRAINT release_year_ck CHECK (release_year > 0),
    CONSTRAINT fk_board_game_categories
        FOREIGN KEY (category_id)
            REFERENCES categories(id)
                ON UPDATE CASCADE
                    ON DELETE CASCADE,

    CONSTRAINT fk_board_game_publishers
        FOREIGN KEY (publisher_id)
            REFERENCES publishers(id)
                ON UPDATE CASCADE
                    ON DELETE CASCADE,

    CONSTRAINT fk_board_game_players_ranges
